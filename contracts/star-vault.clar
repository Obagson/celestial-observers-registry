...
;; STAR-VAULT CONTRACT
;; =========================================================
;; A decentralized registry for amateur astronomers to record
;; and verify astronomical observations permanently on the
;; Stacks blockchain, creating a community-validated archive
;; of celestial discoveries and observations.
;; =========================================================
...
(define-map astronomer-badges
  { astronomer: principal, badge-id: uint }
  {
    earned-at: uint,
...
(define-map astronomer-object-types
  { astronomer: principal, object-type: (string-utf8 50) }
  { count: uint }
)
...
;; Checks if a principal is a registered astronomer
(define-private (is-registered (address principal))
  (default-to false (map-get? astronomers { address: address }))
)

;; Get the current observation count for an astronomer
(define-private (get-observation-count (address principal))
  (default-to u0 (get observation-count (map-get? astronomers { address: address })))
)

;; Get the current verification count for an astronomer
(define-private (get-verification-count (address principal))
  (default-to u0 (get verification-count (map-get? astronomers { address: address })))
)

;; Increment observation count for an astronomer
(define-private (increment-observation-count (address principal))
  (let ((current-count (get-observation-count address)))
    (map-set astronomers
      { address: address }
      (merge (default-to
        {
        ...
        { observation-count: (+ current-count u1) }
      )
    )
  )
)

;; Increment verification count for an astronomer
(define-private (increment-verification-count (address principal))
  (let ((current-count (get-verification-count address)))
    (map-set astronomers
      { address: address }
      (merge (default-to
        {
...
(define-private (update-object-type-count (observer principal) (object-type (string-utf8 50)))
  (let ((current-entry (map-get? astronomer-object-types { astronomer: observer, object-type: object-type })))
    (match current-entry
      existing-entry (map-set astronomer-object-types
        { astronomer: observer, object-type: object-type }
...
(define-private (has-badge (astronomer principal) (badge-id uint))
  (is-some (map-get? astronomer-badges { astronomer: astronomer, badge-id: badge-id }))
)

;; Award a badge to an astronomer
(define-private (award-badge (astronomer principal) (badge-id uint) (observation-id (optional uint)))
  (if (has-badge astronomer badge-id)
    false
    (map-set astronomer-badges
      { astronomer: astronomer, badge-id: badge-id }
      { earned-at: block-height, linked-observation: observation-id }
    )
  )
)
...
;; Check eligibility for "Deep Sky Enthusiast" badge (10+ deep sky objects)
(define-private (check-deep-sky-badge (astronomer principal))
  (let (
    (deep-sky-count (default-to { count: u0 } 
      (map-get? astronomer-object-types { astronomer: astronomer, object-type: "Deep Sky" })))
  )
    (if (>= (get count deep-sky-count) u10)
      (award-badge astronomer u3 none)
      false
    )
  )
)
...
(define-private (check-achievements (astronomer principal))
  (begin
    ...
    (check-deep-sky-badge astronomer)
    true
  )
)
...
(define-read-only (is-verified-by (observation-id uint) (verifier principal))
  (default-to false 
    (get verified (map-get? verifications { observation-id: observation-id, verifier: verifier })))
)
...
(define-read-only (get-astronomer-badge (astronomer principal) (badge-id uint))
  (map-get? astronomer-badges { astronomer: astronomer, badge-id: badge-id })
)

;; Get the number of observations of a specific object type by an astronomer
(define-read-only (get-object-type-count (astronomer principal) (object-type (string-utf8 50)))
  (default-to { count: u0 } 
    (map-get? astronomer-object-types { astronomer: astronomer, object-type: object-type }))
)
...
(define-public (register-astronomer (username (string-utf8 50)))
  (let ((sender tx-sender))
    (asserts! (not (is-registered sender)) (err ERR-ALREADY-REGISTERED))
...
(define-public (record-observation
  ...
  (let (
    ...
    (asserts! (is-registered sender) (err ERR-NOT-REGISTERED))
    
    ;; Validate basic parameters
    (asserts! (and (> (len celestial-object) u0) (> (len object-type) u0)) (err ERR-INVALID-PARAMETERS))
    ...
    ;; Update astronomer's stats
    (increment-observation-count sender)
...
(define-public (verify-observation (observation-id uint))
  (let (
    ...
    (let ((unwrapped-observation (unwrap! observation (err ERR-INVALID-OBSERVATION))))
      ...
      (asserts! (not (is-eq sender (get observer unwrapped-observation))) (err ERR-CANNOT-VERIFY-OWN))
      
      ;; Check that this verifier hasn't already verified this observation
      (asserts! (not (is-verified-by observation-id sender)) (err ERR-ALREADY-VERIFIED))
      
      ;; Record the verification
      (map-set verifications
        { observation-id: observation-id, verifier: sender }
        { verified: true }
      )
      ...
      (map-set observations
        { id: observation-id }
        (merge unwrapped-observation 
          { verification-count: (+ (get verification-count unwrapped-observation) u1) }
        )
      )
      
      ;; Update verifier's stats
      (increment-verification-count sender)
      ...
      (if (is-eq (+ (get verification-count unwrapped-observation) u1) u10)
        (award-badge (get observer unwrapped-observation) u4 (some observation-id))
        false
      )
...
;; Initialize the contract with default badges
;; Note: Would be called once after contract deployment
(define-public (initialize-badges)
  (begin
    ;; Only contract owner can initialize - use a proper authorization pattern in production
    (asserts! (is-eq tx-sender tx-sender) (err ERR-NOT-AUTHORIZED))
    
    ;; Create default badges
    (create-badge "Stargazer" "Registered as an astronomer in the Celestial Observer's Registry" "Register as an astronomer" "Common")
    (create-badge "Observer" "Recorded 5 or more astronomical observations" "Record 5 observations" "Common")
    (create-badge "Planetary Explorer" "Observed all major planets in our solar system" "Observe Mercury, Venus, Mars, Jupiter, Saturn, Uranus, and Neptune" "Rare")
    (create-badge "Deep Sky Enthusiast" "Observed 10 or more deep sky objects" "Record 10 deep sky object observations" "Uncommon")
    (create-badge "Community Validated" "One of your observations received 10 verifications" "Get 10 verifications on a single observation" "Rare")
    (create-badge "Verifier" "Verified 10 or more observations from other astronomers" "Verify 10 observations" "Uncommon")
